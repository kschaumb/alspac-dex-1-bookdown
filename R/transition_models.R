...
ex_group_girls_covs_1 <- msm(exercise_group ~ age, subject = id, data = transition_data_girls, qmatrix = qmat_1, gen.inits = TRUE, control = list(fnscale =5000, maxit = 100000, reltol = 1e-16), covariates = ~ ti_mean_14_std + body_sat_mean_14_std + fear_wtgain_14 + bmi_z_bestavail.13 + parent_highest_occupation)
ex_group_girls_covs_2 <- msm(exercise_group ~ age, subject = id, data = transition_data_girls, qmatrix = qmat_3, gen.inits = TRUE, control = list(fnscale =5000, maxit = 1000000, reltol = 1e-16), covariates = ~ ti_mean_14_std + bmi_z_bestavail.13 + fear_wtgain_14 + body_sat_mean_14_std + parent_highest_occupation, exacttimes = TRUE)
...
resave(ex_group_girls_covs_2, file = 'models/transition_models')
...
ex_group_boys_covs <- msm(exercise_group ~ age, subject = id, data = transition_data_boys, qmatrix = qmat_1, gen.inits = TRUE, control = list(fnscale =5000, maxit = 1000000, reltol = 1e-16), covariates = ~ ti_mean_14_std + bmi_z_bestavail.13 + fear_wtgain_14)
ex_group_boys_covs_2 <- msm(exercise_group ~ age, subject = id, data = transition_data_boys, qmatrix = qmat_3, gen.inits = TRUE, control = list(fnscale =5000, maxit = 1000000, reltol = 1e-16), covariates = ~ ti_mean_14_std + bmi_z_bestavail.13 + fear_wtgain_14 + body_sat_mean_14_std + parent_highest_occupation, exacttimes = TRUE)


resave(ex_group_boys_1, file = 'models/transition_models')
resave(ex_group_boys_2, file = 'models/transition_models')
resave(ex_group_boys_covs, file = 'models/transition_models')
